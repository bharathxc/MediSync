...
def supervisor_node(state: AgentState) -> dict:
    """
    The Supervisor analyzes the user's message and decides whether to
    call tools or respond directly. It also applies PII masking.
    """
    messages = state["messages"]
    member_info = state.get("member_info")
    plan_type = state.get("plan_type")

    # Apply PII masking to the latest user message
    pii_detections = []
    last_msg = messages[-1]
    if isinstance(last_msg, HumanMessage):
        masked_content, pii_detections = mask_pii(last_msg.content)
        if pii_detections:
            # Replace the message with masked version
...
def response_node(state: AgentState) -> dict:
    ...
    Updates member_info if a member lookup was performed.
    """
    import json
    ...
    # Check if we got member details from a tool call
    ...
    # Build system context for final response
    system_content = SYSTEM_PROMPT
    if new_member_info:
        system_content += f"\n\nIdentified Member:\n"
...
def build_agent_graph():
    ...
    workflow.add_conditional_edges(
        ...
        {
            "tools": "tools",
            "respond": END,  # Direct response (no tools needed)
        }
    )

    # After tools → synthesize response
